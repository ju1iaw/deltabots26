...
def unpack_hub_capabilities(data: bytes) -> Tuple[int, HubCapabilityFlag, int]:
    ...
    """
    if len(data) < 2:
        return 0, HubCapabilityFlag(0), 0

    max_char_size = unpack("<H", data[:2])[0]
    flags = unpack("<I", data[2:6])[0] if len(data) >= 6 else 0
    max_user_prog_size = unpack("<I", data[6:10])[0] if len(data) >= 10 else 0
